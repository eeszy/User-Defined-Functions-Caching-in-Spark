...
case class PartitionProject(projectList: Seq[Expression], child: SparkPlan) extends UnaryNode {
  ...
  def generateIterator(input: Iterator[Row]): Iterator[Row] = {
    ...
    val Generator = CS143Utils.getNewProjection(projectList, child.output)

    /* IMPLEMENT THIS METHOD */
    val partitions = DiskHashedRelation.apply(input, Generator).getIterator()
    var t: Iterator[Row] = null
    ...
    new Iterator[Row] {
      ...
      private def fetchNextPartition(): Boolean  = {
        ...
        if (!partitions.hasNext) {
          ...
          t = CS143Utils.generateCachingIterator(projectList, child.output)(partitions.next().getData())
          hasNext()
        }
      }
    }
