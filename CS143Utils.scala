...
object CS143Utils {
  ...
  def getUdfFromExpressions(expressions: Seq[Expression]): ScalaUdf = {
    ...
    def getMyUdf(remainingExpressions: Seq[Expression]): ScalaUdf = {
      remainingExpressions match {
        case Nil          => null
        case head :: tail => head match {
          case u: ScalaUdf => u
          case otherwise   => getMyUdf(tail)
        }
      }
    }
    getMyUdf(expressions.reverse)
  }
  ...
  def apply(
    ...
    new Iterator[Row] {
      ...
      def next() = {
        /* IMPLEMENT THIS METHOD */
        if(input.hasNext) {
          val Next: Row = input.next()
          val a: Row = cacheKeyProjection.apply(Next)
          val b: Row = preUdfProjection.apply(Next)

          val c: Row = {
            if (cache.containsKey(a)) {
              cache.get(a)
            } else {
              val add: Row = udfProject.apply(Next)
              cache.put(a, add)
              add
            }
          }

          val d: Row = postUdfProjection.apply(Next)
          Row.fromSeq(b ++ c ++ d)
        } else {
          null
         }
...
object AggregateIteratorGenerator {
  def apply(resultExpressions: Seq[Expression],
    ...
    new Iterator[Row] {
      ...
      def next() = {
        /* IMPLEMENT THIS METHOD */
        val Entry: (Row, AggregateFunction) = input.next()
        val Group: Row = Entry._1
        val Example: AggregateFunction = Entry._2

        val res = new GenericMutableRow(1)
        res(0) = Example.eval(EmptyRow)

        val RowJoin = new JoinedRow4
        postAggregateProjection(RowJoin(res, Group))
