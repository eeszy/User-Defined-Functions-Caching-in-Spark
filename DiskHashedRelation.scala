...
private[sql] class DiskPartition (
  ...
  def insert(row: Row) = {
    /* IMPLEMENT THIS METHOD */
      if (inputClosed) throw new SparkException("inputHasClosed")
      val Size = measurePartitionSize()
      data.add(row)
      if (Size > blockSize) {
...
private[sql] object DiskHashedRelation {
  ...
  def apply (
    ...
    val partitions = Array.fill(size) {
      cnt += 1
      new DiskPartition("block" + cnt.toString(), blockSize)
    }
    while (input.hasNext) {
      var currentrow = input.next()
      var Hans = keyGenerator(currentrow).hashCode() % size
      partitions(Hans).insert(currentrow)
    }
